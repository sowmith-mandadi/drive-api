...
class ContentService:
    ...
    def create_content(self, content_data: ContentCreate) -> ContentInDB:
        ...
        if not content:
            ...
            logger.warning(f"Failed to create content in repository, using local object")
            now = datetime.now()
            content = ContentInDB(
                id=content_id,
    ...
    def search_content(self, query: str, filters: Dict[str, Any] = None) -> List[ContentInDB]:
    ...
    def update_content_file(self, content_id: str, file_path: str, 
                           extracted_text: str = None, page_content: Dict[str, str] = None) -> bool:
        ...
        if extracted_text and page_content:
            success = self.repository.update_extracted_text(content_id, extracted_text, page_content)
            if not success:
                return False
